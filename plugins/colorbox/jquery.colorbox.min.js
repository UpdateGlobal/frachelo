(function (e, t, n) {
    ...
    function tt() {
        ...
        for (t in B) {
            if (e.isFunction(B[t]) && t.slice(0, 2) !== "on") {
                B[t] = B[t].call(R)
            }
        }
    ...
    function it(t) {
        if (!V) {
            ...
            if (!W) {
                ...
                g.show();
                if (B.returnFocus) {
                    e(R).blur().one(c, function () {
                        e(this).focus()
                    })
                }
                m.css("cursor", B.overlayClose ? "pointer" : "auto");
                m.show();
                m.css("opacity", B.opacity);
                m.hide();
                m.fadeIn(400);
                g.hide();
                g.fadeIn(400);
    ...
    function st() {
        if (!g && t.body) {
            ...
            y.append(G(K).append(G(K, "TopLeft"), w = G(K, "TopCenter"), G(K, "TopRight")), G(K, false, "clear:left").append(E = G(K, "MiddleLeft"), b, S = G(K, "MiddleRight")), G(K, false, "clear:left").append(G(K, "BottomLeft"), x = G(K, "BottomCenter"), G(K, "BottomRight"))).find("div div").css({
                "float": "left"
            });
            k = G(K, false, "position:absolute; width:9999px; visibility:hidden; display:none");
    ...
    function ot() {
        if (g) {
            if (!Q) {
                Q = true;
                j = w.height() + x.height() + b.outerHeight(true) - b.height();
                F = E.width() + S.width() + b.outerWidth(true) - b.width();
                I = C.outerHeight(true);
                q = C.outerWidth(true);
                g.css({
                    "padding-bottom": j,
                    "padding-right": F
    ...
    J.position = function (e, t) {
        ...
        if (B.right !== false) {
            r += Math.max(N.width() - B.w - q - F - Z(B.right, "x"), 0)
        } else if (B.left !== false) {
            r += Z(B.left, "x")
        } else {
            r += Math.round(Math.max(N.width() - B.w - q - F, 0) / 2)
        }
        if (B.bottom !== false) {
            n += Math.max(N.height() - B.h - I - j - Z(B.bottom, "y"), 0)
        } else if (B.top !== false) {
            n += Z(B.top, "y")
        } else {
            n += Math.round(Math.max(N.height() - B.h - I - j, 0) / 2)
        }
        g.css({
            top: i.top,
            left: i.left
    ...
    J.resize = function (e) {
        if (W) {
            ...
            if (e.innerWidth) {
                B.w = Z(e.innerWidth, "x")
            }
            C.css({
                width: B.w
            });
            if (e.height) {
                B.h = Z(e.height, "y") - I - j
            }
            if (e.innerHeight) {
                B.h = Z(e.innerHeight, "y")
            }
    ...
    J.prep = function (t) {
        ...
        if (d) {
            e("select").not(g.find("select")).filter(function () {
    ...
    J.load = function (t) {
        ...
        B.mh = B.h;
        if (B.maxWidth) {
            B.mw = Z(B.maxWidth, "x") - q - F;
            B.mw = B.w && B.w < B.mw ? B.w : B.mw
        }
        if (B.maxHeight) {
            B.mh = Z(B.maxHeight, "y") - I - j;
        ...
        if (B.inline) {
            G(K).hide().insertBefore(e(n)[0]).one(h, function () {
            ...
            e(z = new Image).addClass(s + "Photo").error(function () {
                B.title = false;
                i(G(K, "Error").text("This image could not be loaded"))
            }).load(function () {
                var e;
                z.onload = null;
                if (B.scalePhotos) {
                    ...
                    if (B.mw && z.width > B.mw) {
                        e = (z.width - B.mw) / z.width;
                        r()
                    }
                    if (B.mh && z.height > B.mh) {
                        e = (z.height - B.mh) / z.height;
                        r()
                    }
                }
                if (B.h) {
                    z.style.marginTop = Math.max(B.h - z.height, 0) / 2 + "px"
                }
                if (T[1] && (B.loop || T[U + 1])) {
                ...
                if (p) {
                    z.style.msInterpolationMode = "bicubic"
                }
                setTimeout(function () {
                    i(z)
                }, 1)
            ...
            k.load(n, B.data, function (t, n, r) {
                i(n === "error" ? G(K, "Error").text("Request unsuccessful: " + r.statusText) : e(this).contents())
            })
        }
    };
    J.next = function () {
        if (!X && T[1] && (B.loop || T[U + 1])) {
            U = Y(1);
            J.load()
        }
    };
